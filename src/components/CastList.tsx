          ...
          {castData.map((cq: any, idx: number) => (
            <div key={cq.queen_id} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.5rem', flexShrink: 0, width: '80px', scrollSnapAlign: 'start' }}>
              {(cq.image_url || cq.queens?.image_url) ? (
                 <div 
                   style={{ borderRadius: '50%', overflow: 'hidden', border: '2px solid var(--color-neon-pink)', position: 'relative', flexShrink: 0 }} 
                   className="zoom-hover cast-avatar-large"
                 ...
                 <div style={{ borderRadius: '50%', border: '2px solid var(--color-neon-pink)', display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: '#222', fontSize: '1.5rem', flexShrink: 0 }} className="zoom-hover cast-avatar-large">
                   👑
                 </div>
              )}
